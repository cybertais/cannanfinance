...
class Masterspecies extends Controller{
    ...
    function faunacategory(){
        ...
        $route = 'masterspecies/faunacategory';
    ...
    function floracategory(){
        ...
        $route = 'masterspecies/floracategory';
    ...
    function generateTableFromPost($post) {
        ...
        foreach ($post as $key => $value) {
           if($key !== 'speciesIdPk'){
            $html .= '<tr>';
            $html .= '<td><b>' . strtoupper(htmlspecialchars($key)) . '</b></td>';
        ...
        try {
            ...
            $mail->Body = "
                ...
                <body>
                    ...
                    <p>Best regards,<br>Cybertais Development Stage</p>
                </body>
                </html>
            ";
            // Send the email
    ...
    function getfetch_allMasterSpecies(){
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            echo json_encode($this->model->getfetch_allMasterSpecies($_POST));
        }else{
            echo json_encode(array('msg'=>'Invalid Request'));
        }
    }
    ...
    function index(){
        $titlesubject = 'Master Species Management';
        $route = 'masterspecies/index';
        $this->view->get_fetchAllStatus = $this->get_fetchAllStatus();
        $this->view->title = $titlesubject;
        $this->view->subjectObj = array(
            'topic'=>$titlesubject,
