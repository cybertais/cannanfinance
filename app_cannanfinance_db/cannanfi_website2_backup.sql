...
INSERT INTO `userprofile` VALUES (10,1,'administrator','f9336f4841ef3cfb9c12418503ab35e938e96d64513c7b6d4c9f8d8a3f2d76b9','2026-03-31 12:40:52','Active',0,0,'2025-12-27 18:18:48','2025-12-27 18:16:09','2026-03-31 12:40:52');
/*!40000 ALTER TABLE `userprofile` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Dumping events for database 'cannanfi_website2'
--

--
-- Dumping routines for database 'cannanfi_website2'
--
/*!50003 SET @saved_sql_mode       = @@sql_mode */ ;
/*!50003 SET sql_mode              = 'NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_create_agent` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_create_agent`(
    IN p_fname VARCHAR(256),
    IN p_lname VARCHAR(256),
    IN p_gender VARCHAR(256),
    IN p_email VARCHAR(256),
    IN p_phone VARCHAR(256),
    IN p_agentcode VARCHAR(256),
    IN p_isactive TINYINT,
    IN p_provinceid INT
)
BEGIN
    -- Declare variables to hold the newly generated IDs
    ...
    BEGIN
        -- If an error occurs, rollback the transaction to undo any partial inserts
        ROLLBACK;
        
        -- Return a failure response
        SELECT 
            0 AS success, 
            'Database error occurred. Transaction rolled back.' AS message, 
            NULL AS person_id, 
            NULL AS agent_id;
    END;
    ...
    -- 1. Insert the new person into the `peoples` table
    INSERT INTO `peoples` (`fname`, `lname`, `gender`, `email`, `phone`) 
    VALUES (p_fname, p_lname, p_gender, p_email, p_phone);

    -- Capture the auto-incremented primary key from the `peoples` insert
    SET v_person_id = LAST_INSERT_ID();

    -- 2. Insert the new agent into the `agents` table, linking the personidfk
    INSERT INTO `agents` (
        `personidfk`, 
        `agentcode`, 
        `datecreate`, 
    ...
    -- Capture the auto-incremented primary key from the `agents` insert
    SET v_agent_id = LAST_INSERT_ID();

    -- Commit the transaction if both inserts succeeded
    COMMIT;

    -- Return a success response with the new IDs
    SELECT 
        1 AS success, 
        'Agent successfully created.' AS message, 
...
/*!50003 SET @saved_sql_mode       = @@sql_mode */ ;
/*!50003 SET sql_mode              = 'NO_ZERO_IN_DATE,NO_ZERO_DATE,NO_ENGINE_SUBSTITUTION' */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_edit_agent` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET collation_connection  = utf8mb4_unicode_ci */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_edit_agent`(
    IN p_agentid INT,
    IN p_fname VARCHAR(256),
    IN p_lname VARCHAR(256),
    IN p_gender VARCHAR(256),
...
BEGIN
    ...
    -- Exit handler for SQL exceptions to prevent partial updates
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SELECT 0 AS success, 'System error: Failed to update agent.' AS message;
    END;

    START TRANSACTION;

    -- 1. Retrieve the linked person ID from the agents table
    SELECT personidfk INTO v_personid 
    FROM agents 
    WHERE agentidpk = p_agentid;

    -- 2. Verify the agent exists before attempting updates
    IF v_personid IS NOT NULL THEN
        
        -- Update the individual's personal records in the `peoples` table
        UPDATE peoples
        SET fname = p_fname,
            lname = p_lname,
            gender = p_gender,
            email = p_email,
            phone = p_phone
        WHERE persidpk = v_personid;

        -- Update the agent-specific records in the `agents` table
        UPDATE agents
        SET agentcode = p_agentcode,
            isactive = p_isactive,
            provinceid = p_provinceid,
            datemodified = NOW()
        WHERE agentidpk = p_agentid;

        -- Commit changes and return success
        COMMIT;
        SELECT 1 AS success, 'Agent updated successfully.' AS message;
        
    ELSE
        -- Rollback if the agentidpk wasn't found
        ROLLBACK;
        SELECT 0 AS success, 'Agent not found in the system.' AS message;
    END IF;
...
-- Dump completed on 2026-03-31 12:53:21
